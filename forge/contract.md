...
- **게임이 지금 어느 상태인지 한눈에 달라 보여야 한다.** 게임을 "재생/시연 중, 입력 대기(내 차례), 오답/실패 처리 중, 라운드·판 완료, 게임 종료" 같은 상태로 나눠보고, 각 상태를 스크린샷 한 장씩 나란히 놓았다고 상상해라 — 색·테두리·모양·애니메이션 중 최소 하나가 상태마다 달라야 한다. 텍스트 한 줄만 다르고 나머지 그림은 똑같으면 반려감이다. 체크리스트: (1) 지금 내가 뭘 봐야 하는 시점인지(플레이어 입력을 받지 않는 시연/연출 구간), (2) 지금 내가 뭘 해야 하는 시점인지(입력이 열린 시점)가 색이나 모양만으로 구별되는가?
- **플레이어가 유발한 모든 상태 전이는 그 프레임에 즉시 반응이 있어야 한다.** 탭/스와이프/방향 입력이 규칙을 바꿨다면(정답이든 오답이든, 턴이 넘어갔든, 목숨이 줄었든) 그 프레임에 소리·파티클·화면 흔들림 중 최소 하나가 반드시 난다. "판정은 맞게 처리됐는데 화면은 조용히 그대로"인 순간이 하나라도 있으면 반려 — 자동 계약 테스트는 판정 로직만 보고 이 침묵을 못 잡는다.
- **반례(반려된 실제 사례, 비난이 아니라 패턴으로 기록):**
  ...
  - `memory-sequence`(그림 기억): 규칙은 정상 동작했고 계약 테스트도 전부 통과했지만, 도형이 빛나고 나면 화면이 조용히 입력을 기다렸을 뿐 시작·완료·정답·오답 중 무엇도 신호가 없었다 — 8세 플레이어가 "언제 시작하는지, 맞았는지 틀렸는지 전혀 모르겠다"고 반응했다. 판정 로직이 맞다고 계약 테스트가 통과시켜준 것과, 화면이 그 판정을 보여주는 것은 별개다. 고친 내용: 재생 시작 전 정지 신호('ready' 단계), 입력이 열리는 순간의 색 전환+확산 링, 정답 탭마다 도형 강조, 오답마다 목숨 아이콘이 실제로 팝하며 꺼지는 애니메이션, 라운드 완료 전용 축하 연출 — 전부 상태 전이 하나하나에 대응한다.
  - 셋 다 규칙 자체는 정상 작동했다(계약 테스트 통과) — 문제는 로직이 아니라 **한눈에 읽히는 형태(shape)**였다. 이 프로젝트가 원하는 건 "규칙이 복잡해도 잘 만든 게임"이 아니라 "보자마자 손이 가는 게임"이다.
